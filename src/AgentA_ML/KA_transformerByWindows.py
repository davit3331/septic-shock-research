...
def load_data():
    ...
    train_patient_ids = np.load(DATA_DIR / "train_patient_ids.npy", allow_pickle=True)

    # FIX (sliding windows): a single patient can now contribute MULTIPLE
    # overlapping windows to X_train_full (prepare_transformer_data.py's
    # sliding-window step). A plain row-level train_test_split could put two
    # windows from the SAME patient on opposite sides of this split — the
    # model would then partly "recognize" a patient it's supposedly never
    # seen during validation, which is the same leakage risk we avoided
    # between train and test, just one level down. GroupShuffleSplit keeps
    # every window belonging to one patient entirely on one side.
    gss = GroupShuffleSplit(n_splits=1, test_size=VAL_SIZE, random_state=SEED)
    train_idx, val_idx = next(gss.split(X_train_full, y_train_full, groups=train_patient_ids))
