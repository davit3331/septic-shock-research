...
def build_contextualized_vitals(row, chosen_cols):
    ...
    """
    # col_map is a lookup table — maps each column name to its interpretation function
    col_map = {
    ...
    # lines will collect all the clinical English strings, one per column
    lines = []

    # Loop through each column we want to include
    for col in chosen_cols:

        # Only process this column if we have an interpreter for it
        if col in col_map:

            # Get the raw value from this row — convert NaN to None
            raw = clean_value(row[col]) if col in row.index else None

            # Look up the right interpreter and call it with the raw value
            lines.append(col_map[col](raw))

    # Join all strings with newline — this becomes the clinical summary block
    return "\n".join(lines)
...
def build_fewshot_examples(df, grouped, exclude_ids, n_per_class=1):
    ...
    # Get one no-sepsis and one sepsis patient not in the test set
...
import random
all_patient_ids = list(grouped.groups.keys())
random.seed(42)
random.shuffle(all_patient_ids)
patient_ids = all_patient_ids[:MAX_PATIENTS]
