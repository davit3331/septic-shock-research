...
#### Only: raw vitals + FP-Growth rules + discretized categories
...
OUTPUT_CSV = f"gemini_agentA_datadriven_results_{run_timestamp_file}.csv"
REPORT_FILE = f"gemini_agentA_datadriven_report_{run_timestamp_file}.txt"
...
# Matches how rules were mined (one row per patient, worst-case values)
# =========================
...
# Just shows raw numbers, no interpretation
# =========================
VITAL_COLS = ["HR", "O2Sat", "Temp", "SBP", "MAP", "DBP", "Resp", "Age", "Gender", "HospAdmTime", "ICULOS"]

def format_raw_vitals(row):
    """Format raw vitals as plain numbers — no clinical interpretation."""
    lines = []
...
# =========================
# PROMPT BUILDERS — DATA-DRIVEN ONLY
# No clinical English, no few-shot examples
# Only raw numbers + rule matches
# =========================
def build_full_timeseries_prompt(patient_df, patient_categories, matching_rules):
...
Task:
Using ONLY the data-driven rules above and the raw vitals, predict whether this patient will develop sepsis.
...
def build_final_timestep_prompt(patient_df, patient_categories, matching_rules):
...
Task:
Using ONLY the data-driven rules above and the raw vitals, predict whether this patient has sepsis.
...
# =========================
# RUN EXPERIMENT — NO SHUFFLE (same patients as document 30 / Agent B)
# =========================
results = []
grouped = df.groupby("Patient_ID")
patient_ids = list(grouped.groups.keys())[:MAX_PATIENTS]

print(f"\nRunning Agent A (data-driven only) on {len(patient_ids)} patients...")
print(f"Model: {MODEL_NAME} | Rules: {len(RULES)} | No clinical context\n")
...
for idx, patient_id in enumerate(patient_ids, start=1):
    ...
    # Aggregate vitals and match rules
    ...
    # Full timeseries prompt
    full_prompt = build_full_timeseries_prompt(patient_df, patient_categories, matching_rules)
    try:
    ...
    # Final timestep prompt
    final_prompt = build_final_timestep_prompt(patient_df, patient_categories, matching_rules)
    try:
...
with open(REPORT_FILE, "w") as f:
    f.write("=" * 60 + "\n")
    f.write("GEMINI AGENT A — DATA-DRIVEN ONLY — REPORT\n")
    f.write("=" * 60 + "\n")
    f.write(f"Model:                  {MODEL_NAME}\n")
    f.write(f"Dataset:                {CSV_PATH}\n")
    f.write(f"Prompt Type:            Agent A Data-Driven Only\n")
    f.write(f"Clinical Context:       NONE\n")
    f.write(f"Few-Shot Examples:      NONE\n")
