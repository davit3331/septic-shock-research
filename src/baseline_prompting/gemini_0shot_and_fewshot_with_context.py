...
# Put your Gemini API key in your shell first:
# export GEMINI_API_KEY="your_key_here"
#API_KEY = os.getenv("GEMINI_API_KEY")  # replace with your actual API key or set it as an environment variable

from google.colab import userdata
API_KEY = userdata.get("GEMINI_API_KEY")

# Current Gemini API docs use the Google GenAI SDK and models.generate_content().
# gemini-2.5-flash is a reasonable current model choice for this kind of task.
MODEL_NAME = "gemini-2.5-flash"

# Input CSV path — using balanced dataset (50/50 sepsis/no-sepsis)
CSV_PATH = "Dataset_balanced.csv"  # change if needed

# Limit patients for testing first. Increase Later.
MAX_PATIENTS = 100

# If a patient has many timesteps, keep only the last N to avoid huge prompts. Change as needed.
MAX_TIMESTEPS_PER_PATIENT = 48  # 48 hours is clinically meaningful
...
OUTPUT_CSV = f"gemini_0shotContextualized_results_{run_timestamp_file}.csv"
REPORT_FILE = f"gemini_0shotContextualized_report_{run_timestamp_file}.txt"
...
# Sort so each patient sequence is in time order
...
## LLM tools dont understand NaN well so lets just convert to 'NA' just so we're safe.
def clean_value(value):
    """
    Convert NaN to 'NA' so the prompt is readable.
    """
    if pd.isna(value):
        return None
    return value
...
# Excludes ICULOS (not in Simran's doc, potential data leakage)
# Excludes Hour (used as timestamp label only, not a clinical variable)
CHOSEN_COLS = ["HR", "O2Sat", "Temp", "SBP", "MAP", "DBP", "Resp", "Age", "Gender", "HospAdmTime"]
...
def build_full_timeseries_prompt(patient_df: pd.DataFrame) -> str:
    ...
    Uses contextualized clinical English instead of raw numbers.
    """
    # Keep only the last N timesteps if sequence is too long. Adjust as needed.
    ...
    prompt = f"""
        ...
        using evidence-based clinical thresholds from the medical literature.
        Each block represents one hour of ICU monitoring.
...
def parse_binary_prediction(text: str):
    ...
    # Fallback: find first standalone 0 or 1
...
patient_ids = list(grouped.groups.keys())[:MAX_PATIENTS]  # limit to MAX_PATIENTS for testing

print(f"\nRunning 0-shot contextualized experiment on {len(patient_ids)} patients...")
print(f"Model: {MODEL_NAME}")
print(f"Dataset: {CSV_PATH}")
print(f"Prompt type: Zero-Shot Contextualized (clinical English)\n")

for idx, patient_id in enumerate(patient_ids, start=1):

    patient_df = grouped.get_group(patient_id).sort_values("Hour")  # get rows for this patient, sorted by Hour

    # Getting the Ground truth at patient level:
    # if any timestep has SepsisLabel=1, mark patient as septic
    true_label = int(patient_df["SepsisLabel"].max())  # if at any time patient had sepsis, this will be 1, otherwise 0.
...
with open(REPORT_FILE, "w") as f:
    f.write("=" * 60 + "\n")
    f.write("GEMINI ZERO-SHOT CONTEXTUALIZED — REPORT\n")
    f.write("=" * 60 + "\n")
    f.write(f"Model:                  {MODEL_NAME}\n")
    f.write(f"Dataset:                {CSV_PATH}\n")
    f.write(f"Prompt Type:            Zero-Shot Contextualized\n")
